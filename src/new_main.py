...
class FineTuneDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask = [0.0] * (len(context_sep) - 1) + [1.0] * (input_len - len(context_sep)) if not self.test else [1.0] * (input_len - 1)
...
def evaluate_model(model, dataloader, tokenizer, sep_token="="):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Evaluating"):
            ...
            predictions = tokenizer.batch_decode(generated_outputs, clean_up_tokenization_spaces=True, keep_special_token=True)
...
def main():
    ...
    train_dataset = FineTuneDataset(data_file = train_file, batch_size=args.batch_size, max_seq_length=args.max_seq_length, test=False, sep_token_id = sep_token_id, tokenizer=tokenizer)
    test_dataset = FineTuneDataset(data_file = test_file, batch_size = 1, max_seq_length=args.max_seq_length, test=True, sep_token_id = sep_token_id, tokenizer=tokenizer)
    ...
    training_losses = train_model(model, train_dataloader, args.epochs, args.batch_size, args.learning_rate, args.grad_accum_steps)
